...
- [ ] Show recordings in the coach dashboard — the UI is built (client sheet's "Voice notes" section), but coach playback fails with "Could not load audio". Root cause found by inspecting `pg_policies` directly: `0002_voice_notes_storage.sql`'s coach-read storage policy calls `storage.foldername(name)` inside a correlated subquery against `profiles` — since `profiles` also has a `name` column, the unqualified `name` silently resolved to `profiles.name` (the agent's display name) instead of the storage object's file path, so the policy always evaluated false. `0009` re-ran the same (buggy) clause and didn't fix it. **Action needed: run `supabase/migrations/0010_fix_coach_voice_note_column_ambiguity.sql`** instead — explicitly qualifies `storage.objects.name` so it can't be shadowed.
