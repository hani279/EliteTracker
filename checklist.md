...
- [ ] Show recordings in the coach dashboard — the UI is built (client sheet's "Voice notes" section), but coach playback fails with "Could not load audio". Diagnosed directly against the live database: the `voice_notes` table row is visible to the coach and the audio object genuinely exists (agent's own access lists/signs it fine) — but the coach's `storage.objects` read comes back empty with no error, meaning `0002_voice_notes_storage.sql`'s coach-read storage policy never took effect. **Action needed: run `supabase/migrations/0009_fix_coach_voice_note_storage_policy.sql`** in the SQL editor (re-asserts that one policy idempotently) — everything else in this feature already works.
