...
## Voice Log — AI transcription + coach inbox (user-directed, not from the original meeting)

"The whole idea of the app is to see these voice messages easier" — three pieces, all shipped client-side and verified against the live (not-yet-migrated) database to confirm every failure path degrades safely:

- [x] Coach can delete a voice note — new `0011_coach_voice_note_delete.sql` (coach DELETE policy on both the `voice_notes` row and the Storage object, explicitly qualifying `storage.objects.name` this time — see the migration's comment). Migration run 2026-08-08; re-verified directly against the live database afterward that the row and the storage object are genuinely gone, not just reporting success. Caught and fixed a real bug of my own before asking for that: a plain RLS-blocked `.delete()` succeeds with zero rows affected instead of erroring, so the original code would have shown "Voice note deleted" even when nothing was — fixed by checking `.select()`'s returned rows.
- [x] Voice notes surface first on the coach dashboard — new card at the very top (above the stat tiles), listing the 15 most recent notes across the whole roster with agent name, play, delete, and (once transcribed) the AI summary instead of a bare "Voice note" label. Cached into `s.recentVoiceNotes` alongside `s.coachRoster` (populated in `pullRoster()`) so it renders synchronously and refreshes on the same boot/manual-refresh cadence as the rest of the dashboard, rather than needing a new async-loading pattern.
- [x] AI transcription + summary — new `transcribe-voice-note` Edge Function (OpenAI Whisper for transcription, `gpt-4o-mini` for a coach-facing summary), triggered automatically ~2s after a successful upload. Writes to `voice_notes.transcript` / `.ai_summary` — existing columns from `0001_init.sql` that had never been used. A manual "retry" link appears on any note still showing "Transcribing…" (covers both a slow/failed automatic trigger and manual re-runs). Deployed 2026-08-08 — traced one failed test end to end to a real, specific cause (OpenAI account had no billing credits), confirming the whole pipeline (auth check, storage download, Whisper call) was wired correctly even before it could succeed. **Milestone: user tested it live themselves after adding credits and confirmed it works.**
...
## Header + copy polish (user-directed, not from the original meeting)

- [x] Coach Dashboard header → "**\<Coach's name\>** Dashboard"; Tracker header → "**\<Consultant's name\>** Tracker" — computed live from `profile.name` rather than a static title, on the dashboard/tracker screens specifically (Pipeline, Goals, Clients, Alerts, Report keep their plain names)
- [x] Removed the subtitle line under every header (`Real Estate · Name` / `Name · Head Coach`) app-wide — headers are now just the page title and the menu button, no exceptions per mode
- [x] "mtgs" → "meetings" in the Sales vertical's activity labels ("1st meetings booked" / "1st meetings sat") — label text only, not the `mtgsBooked`/`mtgsSat` data keys, so no existing logged data is affected
- [x] Mic button made bigger, sized on the Fibonacci sequence like the rest of the app's scale — it and its icon were already close to Fibonacci numbers (~55/~21), bumped to the next step (89px / 34px)
- [x] Tracker/Report period tabs relabeled from WTD/MTD/YTD to "This week" / "This month" / "This year" (Today unchanged) — display only; the underlying `wtd`/`mtd`/`ytd` keys and `Intel.rangeKeysToDate()` math are untouched, so the data pulled is still exactly week-to-date/month-to-date/year-to-date
