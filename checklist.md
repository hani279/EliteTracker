...
- [x] Coach can delete a voice note — new `0011_coach_voice_note_delete.sql` (coach DELETE policy on both the `voice_notes` row and the Storage object, explicitly qualifying `storage.objects.name` this time — see the migration's comment). **Action needed: run it in the SQL editor.** Caught and fixed a real bug of my own before asking for that: a plain RLS-blocked `.delete()` succeeds with zero rows affected instead of erroring, so the original code would have shown "Voice note deleted" even when nothing was — fixed by checking `.select()`'s returned rows, verified live that it now reports failure honestly instead.
